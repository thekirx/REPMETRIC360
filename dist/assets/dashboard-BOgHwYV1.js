import{r as _,u as f,s as u}from"./auth-C6cLn3GG.js";/* empty css               */import"https://esm.sh/@supabase/supabase-js@2";const m=document.querySelector(".activity-feed"),$=document.querySelector(".dashboard-right .card:first-child");async function w(){const s=await _();s&&(await f(),await Promise.all([g(s.id),y(s.id)]),b(s.id))}async function g(s){const{data:a,error:e}=await u.from("visits").select("id, scheduled_date, status, notes, doctors(name, specialty, clinic_location)").eq("rep_id",s).order("scheduled_date",{ascending:!1}).limit(5);if(e){console.error("Error loading activities:",e);return}if(!a||a.length===0){m.innerHTML=`
